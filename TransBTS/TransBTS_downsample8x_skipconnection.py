...
class TransformerBTS(nn.Module):
    def __init__(
        ...
        self.skip = skip
        self.bc = 16
        self.Unet = Unet(in_channels=self.num_channels, base_channels=self.bc, num_classes=self.num_classes)
        ...
        self.device = device
        if skip==True:
            self.tskip = tskip(self.img_dim, 3, self.bc ,self.sk_e, self.sk_d, self.sk_h, self.md, self.md_mode, self.device)
    ...
    def encode(self, x):
        if self.conv_patch_representation:
            ...
            #print('before bn range: ',torch.max(x), torch.min(x), x.shape)
            x = self.bn(x)
            #print('before relu range: ',torch.max(x), torch.min(x), x.shape)
            x = self.relu(x)
            ##print('before conv_x range: ',torch.max(x), torch.min(x), x.shape)
            x = self.conv_x(x)
            #print('before permute range: ',torch.max(x), torch.min(x), x.shape)
            x = x.permute(0, 2, 3, 4, 1).contiguous()
            #print('before view range: ',torch.max(x), torch.min(x), x.shape)
        ...
        #print('before pos range: ',torch.max(x), torch.min(x), x.shape)
    ...
    def forward(self, x, auxillary_output_layers=[1, 2, 3, 4]):
        ...
        if self.skip==True:
            [x1_1,x2_1,x3_1] = self.tskip([x1_1,x2_1,x3_1], self.device)
...
def TransBTS(img_dim = 96, skip = False, sk_e=4, sk_d=8, sk_h=2, md = [3,3,1,1], md_mode = 'comp', device = 0):
